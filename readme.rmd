...
data=read.csv("fulldata.csv",header = TRUE)
...
data=data[rownames(data)%in%rownames(IUCN_amphibia),]
dim(data)
data[1,]
#filtered data so that only contains rows for species with values listed in IUCN_amphibia

IUCN_amphibia_matched=IUCN_amphibia[rownames(IUCN_amphibia)%in%rownames(data),]
IUCN_amphibia_matched
dim(IUCN_amphibia_matched)
names(IUCN_amphibia_matched)

data[,13]=IUCN_amphibia_matched[,9]
data[,14]=IUCN_amphibia_matched[,10]
names(data)[13]="Red_list_status"
names(data)[14]="Red_list_criteria"
names(data)
